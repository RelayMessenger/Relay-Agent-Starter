...
/**
 * Scripted model. A user message containing MENU_TRIGGER makes a real
 * two-step turn: search_menu first, then a reply quoting the first result's
 * orderLink from the tool output. Anything else replies immediately.
 */
...
const TEST_HISTORY_PATH = "/__test/history";

export default {
  async fetch(request: Request, env: Bindings): Promise<Response> {
    const url = new URL(request.url);
    if (url.pathname === TEST_HISTORY_PATH) {
      const chatId = url.searchParams.get("chatId") ?? "";
      const root = await getAgentByName<Bindings, StarterRelayChatAgent>(env.RelayChat, chatId);
      const messages = await (root as unknown as {
        getMessages(): Promise<Array<{ id: string; role: string; parts: unknown[] }>>;
      }).getMessages();
